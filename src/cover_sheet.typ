#let get-cover-sheet(
  ...
  let div(height: 1em, body) = {
    if body != none {
      box(
        ...
        stroke: if visualise-content-boxes.flag {
          visualise-content-boxes.stroke
        },
        width: 100%,
      )[
        #body
  ...
  let get-content-for-facility(facility) = {
    ...
    if is-not-none-or-empty(facility) {
      if (
        is-not-none-or-empty(facility.name)
          and is-not-none-or-empty(facility.street)
          and is-not-none-or-empty(facility.city)
      ) {
        grid(
          columns: (75%, 25%),
          align: horizon,
  ...
  if is-not-none-or-empty(university) and is-not-none-or-empty(employer) {
    if (
      is-not-none-or-empty(university.name)
        and is-not-none-or-empty(employer.name)
    ) {
      div[
        #h(20pt) #text(
          1.1em,
  ...
  // Information
  div[
    #let get-content(parameter, body) = if is-not-none-or-empty(parameter) {
      v(6pt) + body
    }
    ...
    #grid(
      ...
      [ #get-content(submission-date)[ #get-info-description(
        txt-submission-date,
      ) ] ],
